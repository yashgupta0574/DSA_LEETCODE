...
public:
    ...
    vector<bool>vis;
    void build(Node* root,int pre[],int idx)
    {
        if(root->data>pre[idx] && root->left!=NULL)
        {
          build(root->left,pre,idx);
        }
        else if(root->data>pre[idx] && root->left==NULL){
            root->left=newNode(pre[idx]);
        }
        else if(root->data<pre[idx] && root->right!=NULL){
            build(root->right,pre,idx);
        }
        else root->right=newNode(pre[idx]);
    }
    ...
    Node* post_order(int pre[], int size)
    {
        ...
        for(int i=1;i<size;i++) build(root,pre,i);   
        return root;
    }
};
